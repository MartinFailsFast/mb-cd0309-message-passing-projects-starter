...
class Location(Base):
    ...
    coordinate: Mapped[Point] = mapped_column(Geometry("POINT"), nullable=False)
    creation_time: Mapped[datetime] = mapped_column(DateTime, nullable=False, default=datetime.utcnow)
    
    # _wkt_shape is a class-level variable, not mapped to a database column
    _wkt_shape: str = None
    ...
    def wkt_shape(self) -> str:
        ...
        if not self._wkt_shape:
            point: Point = to_shape(self.coordinate)  # assuming to_shape is a helper function
